...
docker build -f ./DockerFiles/unit-tests/tests.Dockerfile -t "supply-and-demand-unit-tests:1.0.0" .

if docker run --name "supply-and-demand-unit-tests" supply-and-demand-unit-tests:1.0.0 ; then
  echo "tests -- passed"
  docker cp "supply-and-demand-unit-tests":/app/coverage/ test_reports/
  # Clean up
  docker rm  supply-and-demand-unit-tests
  docker rmi supply-and-demand-unit-tests:1.0.0
  exit 0
else
  echo "tests -- failed"
  docker cp "supply-and-demand-unit-tests":/app/coverage/ test_reports/
