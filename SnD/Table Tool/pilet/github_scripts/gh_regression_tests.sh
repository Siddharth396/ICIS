...
gh workflow run test.yaml \
 ...
 --ref main --field name=fe-regression-test
...
do
    ...
    if [ "$STATUS" = "$VARSUCCESS" ]; then
        echo "Pilet regression tests succeeded"
        exit_code=0
        break
    fi

    if [ "$STATUS" = "$VARFAIL" ]; then
        echo "Pilet regression tests failed"
        exit_code=1
        break
    fi
    loop_count=$((loop_count+1))
    if [ $loop_count -eq 100 ]; then
        echo "Pilet regression tests timed out"
        exit_code=1
        break
    fi
...

{
    ...
    last_line=$(tail -n 100 logs/0_fe-regression-test.txt)
    echo "------------------------------------------------------"
    echo "Last 100 lines of 0_fe-regression-test.txt: $last_line"
    echo "------------------------------------------------------"
...
echo "Artifacts URL For Pilet regression Tests  = $ARCHIVE_URL"
