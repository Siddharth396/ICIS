...
DOCKER_IMAGE="snd/sndctl/subscriber-bff"
DOCKER_TAGNAME="build-${SEM_VERSION}"
CODE_FOLDER_PATH="/opt/bff"
WS_INCLUDES_PATTERN="**/*.exe **/*.dll"
WS_PROJECT_NAME="Supply And Demand - Subscriber Bff"

echo "Running WhiteSource scan from ${DOCKER_IMAGE}:${DOCKER_TAGNAME} -> ${CODE_FOLDER_PATH}"

docker load --input subscriberBffBuild.tar
