...
function main {
  ...
  get_tag "sbcore/sbcore-auth-proxy" "${authproxy}"
  export AUTHPROXY_TAG="${tag}"
  echo "Found auth proxy tag: ${tag}"
  ...
  if [[ "$bfftype" == "authoring" ]] ;
  then
    docker compose -f docker-compose-authoring.yml pull
  else
    # Start all the containers apart from the current repo
    docker compose pull
  fi

  if [[ "$bfftype" == "authoring" ]] ;
  then
    docker compose -f docker-compose-authoring.yml up -d
  else
    # Start all the containers apart from the current repo
    docker compose up -d
  fi
