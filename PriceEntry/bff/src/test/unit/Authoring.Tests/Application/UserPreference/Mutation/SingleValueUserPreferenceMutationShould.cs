namespace Authoring.Tests.Application.UserPreference.Mutation
{
    ...
    public class SingleValueUserPreferenceMutationShould : UserPreferenceTestBase
    {
        private readonly List<UserPreferenceInput> userPreferenceInputs;

        public SingleValueUserPreferenceMutationShould(AuthoringBffApplicationFactory factory)
            : base(factory)
        {
        ...
        protected override List<PriceSeriesGrid> PriceSeriesGrids => new()
        {
            ...
            new()
            {
                ...
                PriceSeriesIds = new List<string> { TestSeries.LNG_China_MM2 }
            }
        };
                ...
                new()
                    {
                        ...
                        {
                            new() { Field = "priceSeriesName", DisplayOrder = 0, Hidden = false },
                            new() { Field = "period", DisplayOrder = 1, Hidden = false },
                ...
                new()
                    {
                        ...
                        ColumnInput = new List<ColumnInput>()
                        {
                            new() { Field = "priceSeriesName", DisplayOrder = 0, Hidden = false },
                            new() { Field = "period", DisplayOrder = 1, Hidden = false },
                            new() { Field = "unitDisplay", DisplayOrder = 2, Hidden = false },
                            new() { Field = "price", DisplayOrder = 3, Hidden = false },
                            new() { Field = "status", DisplayOrder = 4, Hidden = false },
                            new() { Field = "priceDelta", DisplayOrder = 6, Hidden = false },
                            new() { Field = "lastAssessmentPrice", DisplayOrder = 5, Hidden = false },
                            new() { Field = "lastAssessmentDate", DisplayOrder = 7, Hidden = true }
                        }
                    }
            };
        }
