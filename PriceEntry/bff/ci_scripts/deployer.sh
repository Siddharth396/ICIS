...
WORKFLOW_IDENTIFIER="prcent-bff-${WORKFLOW_IDENTIFIER}"
echo "Workflow Identifier = $WORKFLOW_IDENTIFIER"

if [ -z "${IMAGE_REPO}" ]
then
  IMAGE_REPO="prcent/${IMAGE_TYPE}-bff"
fi
...
CI_PATH="LexisNexis-RBA/dsg-icis-prcent-bff"
...
gh workflow run deploy.yml \
    ...
    -f ENVIRONMENT=$ENVIRONMENT -f CLUSTER_VERSION=$CLUSTER_VERSION \
    -f IMAGE_TAG=$IMAGE_TAG -f IMAGE_REPO=$IMAGE_REPO \
...
do
    ...
    # this will give about 30 min to deploy
    if [ $loop_count -eq 360 ]; then
